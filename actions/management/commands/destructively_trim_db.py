...
class Command(BaseCommand):
    ...
    def _orphaned_translation_keys(self) -> list[UUID]:
        """
        Return the translation keys of TranslatableObjects that no longer have any live instance.

        A translation key is unique to a single object, so every wagtail_localize row for that key
        belongs to one object (hence one tenant). If the object still exists in *any* locale it
        belongs to a retained tenant and all of its translation data — including pending target
        translations and reusable translation memory — is that tenant's own and must be kept. Only
        objects with no live instance in any locale are orphaned.
        """
        ...
        for content_type_id in content_type_ids:
            ...
            keys = TranslatableObject.objects.filter(content_type_id=content_type_id).values_list('translation_key', flat=True)
            if model is None:
                # Stale content type (model removed from the codebase): the object cannot exist.
    ...
    def delete_orphaned_translation_data(self) -> None:
        ...
        This mirrors that handler's cleanup, applied to every orphaned object at once.

        Only fully-orphaned objects (no live instance in any locale) are cleaned; a retained object's
        translation data is left untouched, so in-progress translations into not-yet-created target
        locales and translation memory for edited source strings are preserved.
        """
        try:
        ...
        orphan_keys = self._orphaned_translation_keys()
        ...
        # Overrides/translations whose context was SET_NULL by an earlier deletion are orphaned (their
        # context object is gone). Remove them first, so the string GC below sees an accurate picture.
        ...
        # them behind. Drop a String only when it is referenced by neither a segment nor a
        # translation: a retained object may keep a translation of a String whose source segment was
        # since edited away (translation memory), and that must not be cascaded away with the String.
        _, by_type = String.objects.filter(segments__isnull=True, translations__isnull=True).delete()
        accumulate(by_type)
    ...
    def prune_unused_common_indicators(self) -> None:
        ...
        # Directly in use: linked to a retained plan (PlanCommonIndicator) or referenced by a
        # surviving Indicator (Indicator.common is on_delete=PROTECT, so those must be kept).
        keep_ids = set(
        ...
        # A kept indicator's normalization and causal-relation metadata lives in through models that
        # cascade on either endpoint, so a normalizer or related indicator that isn't itself directly
        # used must still be kept — deleting it would strip that metadata from the retained indicator.
        # Expand the keep set across those edges to a fixed point, since the dependencies chain.
        frontier = set(keep_ids)
        while frontier:
            reached: set[int] = set()
            reached.update(
                CommonIndicatorNormalizator.objects.filter(normalizable_id__in=frontier).values_list('normalizer_id', flat=True)
            )
            reached.update(
                RelatedCommonIndicator.objects.filter(causal_indicator_id__in=frontier).values_list(
                    'effect_indicator_id', flat=True
                )
            )
            reached.update(
                RelatedCommonIndicator.objects.filter(effect_indicator_id__in=frontier).values_list(
                    'causal_indicator_id', flat=True
                )
            )
            frontier = reached - keep_ids
            keep_ids |= frontier
