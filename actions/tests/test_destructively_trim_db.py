...
def test_delete_orphaned_translation_data_keeps_pending_translation_for_live_source(plan_with_pages):
    from wagtail.models import Locale

    from wagtail_localize.models import StringSegment, StringTranslation, Translation

    page = StaticPageFactory.create(parent=plan_with_pages.root_page)  # live source (English) instance
    source, _ = TranslationSource.update_or_create_from_instance(page)
    da = Locale.objects.get_or_create(language_code='da')[0]
    # An in-progress translation into Danish: the source is live but no Danish instance exists yet.
    translation = Translation.objects.create(source=source, target_locale=da)
    segment = StringSegment.objects.filter(source=source).first()
    assert segment is not None
    danish = StringTranslation.objects.create(
    ...
    # The source object is live, so its pending Danish translation work is preserved, not dropped.
    assert TranslationSource.objects.filter(pk=source.pk).exists()
    assert Translation.objects.filter(pk=translation.pk).exists()
    assert StringTranslation.objects.filter(pk=danish.pk).exists()


def test_delete_orphaned_translation_data_keeps_live_translation_memory(plan_with_pages):
    from wagtail.models import Locale

    from wagtail_localize.models import String, StringSegment, StringTranslation, TranslationContext

    page = StaticPageFactory.create(parent=plan_with_pages.root_page)  # live source object
    source, _ = TranslationSource.update_or_create_from_instance(page)
    da = Locale.objects.get_or_create(language_code='da')[0]
    context = TranslationContext.objects.filter(object_id=page.translation_key).first()
    assert context is not None
    # Obsolete translation memory: a source String no current segment references, but whose
    # translation is retained for reuse/PO export on the still-live object.
    stale = String.objects.create(locale=source.locale, data='Edited-away source text')
    memory = StringTranslation.objects.create(
        translation_of=stale,
        locale=da,
        context=context,
        data='Bevaret oversættelse',
        translation_type=StringTranslation.TRANSLATION_TYPE_MANUAL,
    )
    assert not StringSegment.objects.filter(string=stale).exists()

    Command().delete_orphaned_translation_data()

    # The String has no segment but still has a live-object translation, so it (and the translation)
    # must survive rather than being cascaded away.
    assert String.objects.filter(pk=stale.pk).exists()
    assert StringTranslation.objects.filter(pk=memory.pk).exists()


def test_prune_unused_common_indicators_keeps_indirect_dependencies():
    from indicators.models.common_indicator import CommonIndicatorNormalizator, RelatedCommonIndicator

    # Directly kept because a surviving indicator references it (Indicator.common is PROTECT).
    indicator = IndicatorFactory.create()
    assert indicator.common is not None
    used = indicator.common
    # A normalizer and a causally-related indicator, neither of which is itself linked to a plan or
    # a surviving indicator, so they are only reachable through the kept indicator's metadata.
    normalizer = CommonIndicatorFactory.create()
    normalization = CommonIndicatorNormalizator.objects.create(
        normalizable=used, normalizer=normalizer, unit=used.unit, unit_multiplier=1.0,
    )
    related = CommonIndicatorFactory.create()
    relation = RelatedCommonIndicator.objects.create(
        causal_indicator=used, effect_indicator=related, effect_type=RelatedCommonIndicator.INCREASES,
    )

    Command().prune_unused_common_indicators()

    # The indirectly-referenced indicators are kept, so the kept indicator's normalization and
    # relation metadata survives instead of being cascaded away.
    assert CommonIndicator.objects.filter(pk=normalizer.pk).exists()
    assert CommonIndicator.objects.filter(pk=related.pk).exists()
    assert CommonIndicatorNormalizator.objects.filter(pk=normalization.pk).exists()
    assert RelatedCommonIndicator.objects.filter(pk=relation.pk).exists()
