...
def _get_language_choices():
    ...
    for lang_code, _lang_name in get_available_admin_languages():
        try:
            ...
            lang_info = None
        if lang_info is not None and lang_info['code'] == lang_code.lower():
            lang_name_local = lang_info['name_local']
        else:
            # get_language_info either raised KeyError (language unknown to Django) or fell back to a variant of
            # lang_code. We expect that the local names of such languages are specified manually in the settings.
