...
def test_action_list_page_contains_attribute_type(plan_with_pages, attribute_type, field_name):
    page = plan_with_pages.root_page.get_children().type(ActionListPage).get().specific
    assert not page.contains_attribute_type(attribute_type, field_name)
    setattr(page, field_name, [('attribute', {'attribute_type': attribute_type})])
    page.save()
    page.refresh_from_db()
    assert page.contains_attribute_type(attribute_type, field_name)
...
def test_action_list_page_category_type_in_filters(plan_with_pages, category_type, field_name):
    page = plan_with_pages.root_page.get_children().type(ActionListPage).get().specific
    assert not page.contains_category_type(category_type, field_name)
    setattr(page, field_name, [('category', {'category_type': category_type})])
    page.save()
    page.refresh_from_db()
    assert page.contains_category_type(category_type, field_name)
...
def test_action_list_page_remove_attribute_type(plan_with_pages, attribute_type, field_name):
    page = plan_with_pages.root_page.get_children().type(ActionListPage).get().specific
    page.insert_attribute_type(attribute_type, field_name)
    page.save()
    page.refresh_from_db()
    assert page.contains_attribute_type(attribute_type, field_name)
    page.remove_attribute_type(attribute_type, field_name)
    page.save()
    page.refresh_from_db()
    assert not page.contains_attribute_type(attribute_type, field_name)
...
def test_action_list_page_remove_category_type(plan_with_pages, category_type, field_name):
    ...
    page.refresh_from_db()
    assert page.contains_category_type(category_type, field_name)
    page.remove_category_type(category_type, field_name)
    page.save()
    page.refresh_from_db()
    assert not page.contains_category_type(category_type, field_name)
