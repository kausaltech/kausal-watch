...
### Core Technologies
- **Django 6.1** with Wagtail CMS for content management
- **GraphQL** APIs using both Graphene (legacy) and Strawberry GraphQL.
...
- **Strawberry GraphQL** (modern) - Gradually replacing Graphene. Interop layer (`kausal_common.graphene.strawberry_schema.CombinedSchema`) allows referring to Graphene types from Strawberry types.
...
### Testing Conventions
- If there is a factory (e.g., `ModelFactory`) registered as a fixture (e.g., `model_factory`), prefer creating objects via `ModelFactory.create()` over `model_factory()`. This will allow type checkers to figure out the type of the created object.
...
- Made importable through the committed `src/kausal_watch_extensions` symlink; when the submodule is absent the symlink dangles and the extension is simply off
- Automatically included in tests and URL routing when available
- Client bundles and translations are gitignored build outputs, built by `mise deps` (provider config in `private/extensions/mise/watch.toml`, loaded via `mise/conf.d/`)
...
- Do your best not to use `Any` as a shortcut. Use `TypedDict` instead of `dict[str, Any]` unless that leads to too much repetition or the dict shape is defined in a 3rd party package.
