            ...
            if old_path != new_path:
                ...
                try:
                    with storage.open(old_path, 'rb') as source_file:
                        # `save` may pick a different name than the one we ask for (e.g. when
                        # another file already occupies `new_path`), so record the name it used.
                        saved_path = storage.save(new_path, source_file)
