"""
Assigning responsible parties and contact persons to an action's tasks.

The assignments are a grandchild relation: tasks are themselves edited in a formset nested in the
action edit form, and `ActionEditHandler`/`ActionAdminForm` rebuild and reorder those formsets by
hand, so the admin tests go through the real edit view and the real draft round-trip rather than
exercising the models in isolation.

The rest covers what the relation is reachable through: the field customizations and contact-person
permissions that gate the panels, the GraphQL and REST representations along with the plan's
contact-person redaction, and what serving them costs in queries.
"""
...
def test_admin_post_creates_task_with_assignments(client, action, plan_admin_user):
    ...
    post_data = _base_post_data(action) | {
        'tasks-TOTAL_FORMS': '1',
        'tasks-INITIAL_FORMS': '0',
        'tasks-0-name': 'Insulate the depot',
    ...
    assert task.name == 'Insulate the depot'
