...
def test_person_post_creates_log_entry(api_client, plan, person_list_url, plan_admin_person, organization_factory):
    ...
    response = api_client.post(
        person_list_url + f'?plan={plan.identifier}',
        data=[
            {
                'first_name': 'Test',
                'last_name': 'Person',
...
def test_person_put_creates_log_entry(api_client, plan, person_list_url, plan_admin_person, person_factory):
...
def test_person_delete_creates_log_entry(api_client, plan, plan_admin_person, person_factory):
    ...
    log_entry = PlanScopedModelLogEntry.objects.filter(
        content_type=content_type, object_id=str(person_pk), action='wagtail.delete', plan=plan
    ).first()
...
def test_bulk_person_post_creates_individual_log_entries(api_client, plan, person_list_url, plan_admin_person, person_factory):
...
def test_bulk_person_put_creates_individual_log_entries(api_client, person_list_url, plan, plan_admin_person, person_factory):
